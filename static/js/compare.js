...
$(document).ready(function () {
	...
	function calculate() {
		...
		var slsArray = $slsTextArea.value.split("\n"),
			...
			numberDiff = _.filter(_.intersection(onecKeys, slsKeys), function (num) {
				return slsObject[num] != onecObject[num]
			});

		$slsOnly.innerHTML = _.map(slsOnly, function (num) {
			return '<b>SLS:</b> ' + num + ' -> ' + slsObject[num]
		}).join('<br />');
		$onecOnly.innerHTML = _.map(onecOnly, function (num) {
			return '<b>1C:</b> ' + num + ' -> ' + onecObject[num]
		}).join('<br />');
		$numberDiff.innerHTML = _.map(numberDiff, function (num) {
			return num + ' -> <b>SLS:</b> ' + slsObject[num] + ' -> <b>1C:</b> ' + onecObject[num]
		}).join('<br />');
